 ...
 * $Revision: 1.9 $ $Date: 2012/11/07 21:11:12 $
...
function Toolpath (builder, id) {
    ...
    this.length = parseFloat(el["length"]);

    this.bbox = BoundingBox.fromArray(el["bbox"]);
...
//	    });
	...
	for (var i=0; i<pnodes.length; i++) {
		...
		if (p["t"])
		    this.tsamples[i] = get_float(p["t"]);
		this.dsamples[i] = get_float(p["d"]);
	    }
	    
	    var vec = parse_float_vec(p["l"]);
